...
abstract class Plate extends JPanel {
    ...
    protected void loadImage() {
        try {
            ...
            plateImage = resizeImage(plateImage, 300, 300); 
        } catch (IOException e) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (plateImage != null) {
            g.drawImage(plateImage, 50, 50, this); // Adjusted position to prevent covering fruits
        }
    }
}
...
class PlateWithBanana extends Plate {
    ...
    protected void paintComponent(Graphics g) {
        super.paintComponent(g);
        if (bananaImage != null) {
            g.drawImage(bananaImage, 80, 120, this); // Adjusted position
        }
    }
}
...
class PlateWithOrange extends PlateWithBanana {
    ...
    protected void loadImage() {
        super.loadImage();
        try {
            orangeImage = ImageIO.read(new File("Orange.png"));
    ...
    protected void paintComponent(Graphics g) {
        super.paintComponent(g);
        if (orangeImage != null) {
            g.drawImage(orangeImage, 180, 120, this); // Adjusted position
        }
    }
}
...
class PlateWithApple extends PlateWithOrange {
    ...
    protected void paintComponent(Graphics g) {
        super.paintComponent(g);
        if (appleImage != null) {
            g.drawImage(appleImage, 140, 50, this); // Adjusted position
        }
    }
}
...
public class FruitPlateGUI {
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            descriptionLabel.setHorizontalAlignment(SwingConstants.RIGHT); // Set to top right corner
